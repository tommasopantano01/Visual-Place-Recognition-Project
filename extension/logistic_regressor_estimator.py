"""
Stima la threshold num_inliers per metodo4 — regressione logistica P(hard).

Uso:
    python extension_6_1/logistic_threshold_estimator.py \\
...
Il modello .joblib viene salvato in extension_6_1/models/.
...
# ============================================================
# INVERSIONE ANALITICA DELLA SIGMOIDE PER TROVARE N*
# ============================================================

def find_inlier_threshold(clf, best_tau):
    """
    Risolve P(hard | N*) = best_tau invertendo la sigmoide:
        logit(best_tau) = w * (N* - mean) / std + b
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Stima threshold logistica P(hard) — metodo4"
    )
...
def main(args):
    ...
    model_path = _MODEL_DIR / f"logreg_{args.vpr_method}_{args.matcher.replace('-', '_')}.joblib"
    ...
    print("\n" + "=" * 60)
    print(f"RISULTATO — metodo4 | {args.vpr_method} | {args.matcher}")
