"""
logistic_hard/logistic_hard_calibrate.py — Allena P(hard | num_inliers) e
calibra tau su validation. Scrive threshold.csv + model.json.

Uso:
    python VPR-adaptive-re-ranking/logistic_hard/logistic_hard_calibrate.py \
...
def parse_args():
    parser = argparse.ArgumentParser(description="Calibra logistic P(hard) — metodo5")
...
def main(args):
    ...
    print(f"Train: {len(train_df)} | Val: {len(val_df)} | hard rate train: {train_df['hard'].mean():.3f}")
    ...
    save_model_json(_MODEL_JSON, regressor_to_dict(clf, ["inliers"]))
